...
import com.timofeenkoprojects.tutorbot.entity.user.Action;
import com.timofeenkoprojects.tutorbot.entity.user.Role;
import com.timofeenkoprojects.tutorbot.entity.user.User;
import com.timofeenkoprojects.tutorbot.repository.DetailsRepo;
import com.timofeenkoprojects.tutorbot.repository.TimeTableRepo;
...
public class TimetableManager extends AbstractManager {
    ...
    public TimetableManager(AnswerMethodFactory methodFactory,
                            ...
                            UserRepo userRepo,
                            TimeTableRepo timeTableRepo, DetailsRepo detailsRepo) {
    ...
    public BotApiMethod<?> answerCallbackQuery(CallbackQuery callbackQuery, Bot bot) {
        ...
        if (splitCallbackData.length > 1 && "add".equals(splitCallbackData[1])) {
            ...
            switch (splitCallbackData[2]) {
                case WEEKDAY -> {
                    return adWeekDay(callbackQuery, splitCallbackData);
                }
                case HOUR -> {
    ...
    private BotApiMethod<?> askConfirmation(CallbackQuery callbackQuery, String[] splitCallbackData) {
        return methodFactory.getEditMessageText(
                callbackQuery,
                "Вы уверенны, что хотите удалить запись из расписания?",
    ...
    private BotApiMethod<?> finish(CallbackQuery callbackQuery, String[] splitCallbackData, Bot bot) throws
    ...
    private BotApiMethod<?> back(Message message, String id) {
        return methodFactory.getSendMessage(
                message.getChatId(),
                "Bы можете настроить описание и заголовок",
                keyboardFactory.getInlineKeyboard(
    ...
    private BotApiMethod<?> addMinute(CallbackQuery callbackQuery, String[] splitCallbackData) {
        ...
        for (User user : me.getUsers()) {
            text.add(user.getDetails().getUsername());
    ...
    private BotApiMethod<?> adWeekDay(CallbackQuery callbackQuery, String[] data) {
    ...
    private BotApiMethod<?> add(CallbackQuery callbackQuery, String[] splitCallbackData) {
        ...
        return methodFactory.getEditMessageText(
                callbackQuery,
                """
                        ✏️ Выберите день, в который хотите добавить занятие:
                         """,
                keyboardFactory.getInlineKeyboard(
